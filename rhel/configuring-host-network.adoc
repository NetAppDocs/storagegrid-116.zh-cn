...
xref:creating-node-configuration-files.adoc[正在创建节点配置文件]
...
在安装或启动期间或热添加接口时， Linux 操作系统会自动创建 `ensXYZ` 接口。除了确保接口设置为在启动后自动启动之外，无需进行任何配置。您必须确定哪个 `ensXYZ` 对应于哪个 StorageGRID 网络（网格，管理员或客户端），以便稍后在配置过程中提供正确的映射。
...
xref:example-etc-sysconfig-network-scripts.adoc[示例 /etc/sysconfig/network-scripts]
