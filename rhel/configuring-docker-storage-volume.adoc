...
StorageGRID 系统作为一组容器在 Red Hat Enterprise Linux 或 CentOS 上运行。如果您已选择使用 Docker 容器引擎，请按照以下步骤安装 Docker 。否则， <<Install Podman,安装 Podman>>。
...
StorageGRID 系统作为一组容器在 Red Hat Enterprise Linux 上运行。如果您已选择使用 Podman 容器引擎，请按照以下步骤安装 Podman 。否则， <<Install Docker,安装 Docker>>。
