...
IMPORTANT: 在运行 `sn-recovery-postinstall.sh` 之前，请勿在恢复期间重新启动存储节点（请参见的步骤） <<post_install_script_step,安装后脚本>>）重新格式化故障存储卷并还原对象元数据。在 `sn-recovery-postinstall.sh` 完成之前重新启动存储节点会导致尝试启动的服务出错，并导致 StorageGRID 设备节点退出维护模式。
