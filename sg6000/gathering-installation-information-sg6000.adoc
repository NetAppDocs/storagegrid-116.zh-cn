...
您必须将设备中的两个存储控制器(E2800系列控制器或EF570控制器)连接到要用于SANtricity System Manager的管理网络。控制器位于每个设备中，如下所示：
...
| 所需信息 | 控制器 A 的值 | 控制器 B 的值 
 ...
 a| 
要连接到管理端口1的以太网交换机端口(在E2800A控制器上标记为P1、在E2800B控制器上标记为0a)
 a| 
 a| 
 ...
 a| 
管理端口1的MAC地址(印在E2800A控制器上端口P1附近的标签上、E2800B控制器上的端口0a上)
 a| 
 a| 
